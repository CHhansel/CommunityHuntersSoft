...
const OrderFinalResume = ({ close, order, products, handleSubmitOrder, tables }) => {
      ...
      <ul className="border">
        {order.products.map((product, index) => (
          <li key={product.uniqueId} className=" my-2 py-2 px-5">
            ...
            <div className="ml-5"> <span>Nota:</span> {product.comment || "Sin anotaciónes"}</div>
          </li>
        ))}
      </ul>
      <div>
        ...
        {order.selectedOrderType === "3" && order.selectedTable != null && (
          <div>
            <h4>Mesa: {tables.find(t => t.id == order.selectedTable).number}</h4>
